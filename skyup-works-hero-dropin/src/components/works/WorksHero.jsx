  ...
  return (
    <section className="works-hero relative w-full overflow-hidden bg-[#04050C] font-sans">
      ...
      <style>{`
        ...
        .wh-cue  { opacity:0; animation:wh-rise 1s 2.1s forwards; }
        ...
        @keyframes wh-drop { 0%{top:-40%} 70%,100%{top:110%} }
        .wh-rail { width:1px; height:34px; position:relative; overflow:hidden;
          background:linear-gradient(#9CC0FF,transparent); }
        .wh-rail::after { content:""; position:absolute; top:-40%; left:0; width:1px; height:40%;
          background:#FFB950; animation:wh-drop 1.8s 2.4s infinite cubic-bezier(.6,0,.2,1); }
      ...
      {/* content — left third */}
      <div ref={wrapRef} className="relative z-[5] mx-auto flex h-full max-w-[1240px] flex-col justify-center px-6 sm:px-10 lg:px-[52px]">
        ...
        <h1 className="wh-h1 mt-4 max-w-[15ch] text-4xl font-bold leading-[0.98] tracking-tight text-white sm:text-5xl lg:text-[76px]">
          We run{" "}
      ...
      {/* telemetry */}
      <div className="absolute inset-x-0 bottom-[clamp(66px,9vh,104px)] z-[5] mx-auto grid max-w-[1240px] grid-cols-2 gap-x-6 gap-y-5 px-6 sm:px-10 md:grid-cols-4 lg:px-[52px]">
        {METRICS.map((m, i) => (
          <div
            key={m.label}
      ...
      {/* scroll cue */}
      <div className="wh-cue absolute bottom-6 left-1/2 z-[5] flex -translate-x-1/2 flex-col items-center gap-2 font-mono text-[10.5px] uppercase tracking-[0.34em] text-neutral-400">
        Scroll
        <span className="wh-rail" />
      </div>
    </section>
  );
}
